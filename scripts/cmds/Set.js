module.exports = {
  ...
  onStart: async function ({ args, event, api, usersData }) {
    const permission = ["61585966465927"];
  if (!permission.includes(event.senderID)) {
    api.sendMessage("You don't have enough permission to use this command. Only My Lord bunny Can Use It.", event.threadID, event.messageID);
    return;
  }
    const query = args[0];
